...
export async function runAgentStatusMigrations(knex: Knex): Promise<void> {
  const hasTable = await knex.schema.hasTable("agent_status");
  if (!hasTable) {
  ...
  // Add system_prompt_sources column if missing (existing installs)
  const hasColumn = await knex.schema.hasColumn(
    "agent_status",
    "system_prompt_sources",
  );
  if (!hasColumn) {
    await knex.schema.alterTable("agent_status", (t: any) => {
      t.text("system_prompt_sources").nullable();
    });
  }
}

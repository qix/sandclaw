...
metadata:
  openclaw:
    ...
    requires:
      tools: ["google_workspace_read", "google_workspace_exec"]
---
...
```
google_workspace_read  command: ["sheets", "<resource>", "<method>", ...flags]
google_workspace_exec  command: ["sheets", "<resource>", "<method>", ...flags]
```
...
```
google_workspace_read command: ["sheets", "--help"]
google_workspace_read command: ["schema", "sheets.<resource>.<method>"]
```

Use schema output to build your `--params` and `--json` flags.

...
function VerificationCard({
  ...
  const cardStyle: React.CSSProperties = isResolved
    ? { opacity: 0.55, borderColor: isRejected ? 'rgba(239, 68, 68, 0.35)' : undefined }
    : {};
    ...
    <Card key={r.id} style={cardStyle}>
      <CardHeader style={overlayStyle}>
        ...
        <div className="sc-flex-row">
          {isResolved && (
            <Badge
              bg={isRejected ? 'rgba(239, 68, 68, 0.2)' : 'rgba(139, 143, 163, 0.2)'}
...
function Pagination({ page, totalPages }: { page: number; totalPages: number }) {
  ...
  for (const p of sorted) {
    if (p - prev > 1) {
      items.push(
        <span key={`gap-${p}`} className="sc-pagination-gap">&hellip;</span>,
      );
    }
    items.push(
      <a
  ...
  return (
    <nav className="sc-pagination">
      {page > 1 && (
        <a href={`?page=verifications&historyPage=${page - 1}`} className="sc-pagination-link">&laquo; Prev</a>
      )}
      {items}
      {page < totalPages && (
        <a href={`?page=verifications&historyPage=${page + 1}`} className="sc-pagination-link">Next &raquo;</a>
      )}
    </nav>
  );
}

export function VerificationsPage({ requests, history, renderers }: VerificationsPageProps) {
  return (
    <div className="sc-section">
      ...
      {requests.length === 0 && (
        <p style={{ color: colors.muted, marginBottom: '1.5rem' }}>No pending verification requests.</p>
      )}
      ...
      {history && history.requests.length > 0 && (
        <>
          <div style={{ marginTop: requests.length > 0 ? '2rem' : '0.5rem' }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 600, color: colors.muted, marginBottom: '0.75rem' }}>
              History

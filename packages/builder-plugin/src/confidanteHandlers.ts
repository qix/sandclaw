...
import { BUILDER_CONFIDANTE_JOB_TYPE, DEFAULT_BUILDER_RESULT_JOB_TYPE } from './constants';
...
export function createBuilderConfidanteHandlers(config: BuilderConfidanteConfig) {
  const { workDir } = config;

  return {
    async [BUILDER_CONFIDANTE_JOB_TYPE](ctx: ConfidantePluginContext): Promise<void> {
      ...
      ctx.logger.info('builder.confidante.npm_install', { jobId: ctx.job.id, requestId });
      ...
      ctx.logger.info('builder.confidante.running_pi', { jobId: ctx.job.id, requestId });
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to post builder result (${response.status}): ${body.slice(0, 200)}`);
      }

...
export function registerRoutes(app: any, db: any, pluginConfig: BuilderPluginConfig) {
  ...
  app.post('/request', async (c: any) => {
    ...
    const responseJobType = body.responseJobType || DEFAULT_BUILDER_RESULT_JOB_TYPE;
    const now = Date.now();
  ...
  app.post('/approve/:id', async (c: any) => {
    ...
    if (!request || request.status !== 'pending' || request.plugin !== 'builder') {

    ...
    >
      ...
      <script
        dangerouslySetInnerHTML={{
          __html: `
(function() {
  var fileListEl = document.getElementById('skills-files');
  var filenameEl = document.getElementById('skills-filename');
  var dirtyEl = document.getElementById('skills-dirty');
  var saveBtn = document.getElementById('skills-save-btn');
  var statusEl = document.getElementById('skills-status');
  var editorEl = document.getElementById('skills-editor');
  var newBtn = document.getElementById('skills-new-btn');
  ...
  initScript.textContent = \`
    ...
    var el = document.getElementById('skills-editor');
    ...
    var updateListener = EditorView.updateListener.of(function(update) {
      if (update.docChanged && window.__skillsOnChange) {
        window.__skillsOnChange(update.state.doc.toString());
      }
    });
    ...
    window.__skillsEditor = view;
    window.__skillsEditorState = EditorState;
    window.__skillsEditorExtensions = [
      basicSetup,
      markdown({ codeLanguages: languages }),
      oneDark,
      customTheme,
    ...
    // Signal ready
    if (window.__skillsOnReady) window.__skillsOnReady();
  \`;
  document.body.appendChild(initScript);

  window.__skillsOnChange = function(content) {
    var dirty = content !== savedContent;
    dirtyEl.style.display = dirty ? 'inline' : 'none';
  };

  window.__skillsOnReady = function() {
    editorReady = true;
    loadFileList();
  };

  function setEditorContent(content, readOnly) {
    var view = window.__skillsEditor;
    var State = window.__skillsEditorState;
    var exts = window.__skillsEditorExtensions;
  ...
  function renderFileList(files) {
    ...
    if (files.length === 0) {
      ...
      emptyEl.textContent = 'No skill files yet';
  ...
  function openFile(filePath) {
    ...
    fetch('/api/skills/file?path=' + encodeURIComponent(filePath))
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data.error) {
  ...
  newBtn.addEventListener('click', function() {
    var name = prompt('New skill file name (e.g. MY_SKILL.md):');
    if (!name) return;

  ...
  return (
    <div style={{ padding: "1.5rem" }}>
      ...
      <p style={{ color: colors.muted }}>
        ...
        requires human approval before the Confidante agent executes it. The
        approval and execution are deliberately decoupled: approving once lets
        the Confidante perform multiple searches to fulfil the request.
      </p>
      <section>
        <h3>Flow</h3>
...
export function BrowserVerificationRenderer({
  ...
  const constraints = data?.constraints as
          ...
          {constraints &&
            (constraints.maxSteps != null || constraints.timeoutMs != null) && (
              <tr>
                ...
                >
                  {constraints.maxSteps != null && (
                    <>max {constraints.maxSteps} steps</>
                  )}
                  {constraints.maxSteps != null &&
                    constraints.timeoutMs != null && <>, </>}
                  {constraints.timeoutMs != null && (
                    <>{(constraints.timeoutMs / 1000).toFixed(0)}s timeout</>
                  )}
                </td>
              </tr>
            )}

...
import { BROWSER_CONFIDANTE_JOB_TYPE, DEFAULT_BROWSER_RESULT_JOB_TYPE } from './constants';

export const browserConfidanteHandlers = {
  async [BROWSER_CONFIDANTE_JOB_TYPE](ctx: ConfidantePluginContext): Promise<void> {
    let payload: { requestId: string; prompt: string; responseJobType?: string };
    try {
    ...
    const result = await ctx.docker.run('alpine:latest', ['echo', 'hello from inside docker']);

    if (result.exitCode !== 0) {
      throw new Error(`Docker command failed (exit ${result.exitCode}): ${result.stderr}`);
    }
    ...
    const responseJobType = payload.responseJobType || DEFAULT_BROWSER_RESULT_JOB_TYPE;
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to post browser result (${response.status}): ${body.slice(0, 200)}`);
    }

...
import type { DockerService, DockerRunResult } from '@sandclaw/confidante-plugin-api';
...
export class DockerServiceImpl implements DockerService {
  ...
  async run(
    ...
    return new Promise<DockerRunResult>((resolve, reject) => {
      const proc = execFile('docker', args, { timeout: timeoutMs, maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
        ...
        this.logger.info('docker.run.complete', { image, exitCode, stdoutLen: stdout.length, stderrLen: stderr.length });
